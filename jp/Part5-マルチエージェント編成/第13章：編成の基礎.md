...
複雑度 < 0.3 かつ サブタスク1つ かつ ツール不要? ──はい──► SimpleTaskWorkflow
    ...
    │
    ▼
サブタスク > 5 または 依存あり? ──はい──► SwarmWorkflow
    │                               (複雑なマルチエージェント協調)
    いいえ
    │
    ▼
...
| **Swarm** | 6個以上のサブタスク、複雑な依存、動的協調が必要 | チーム管理、メールボックス通信 |
...
3. **ルーティング決定**：単純タスクはSimpleTask、複雑タスクはDAGかSwarm
...
- [`orchestrator_router.go`](https://github.com/Kocoro-lab/Shannon/blob/main/go/orchestrator/internal/workflows/orchestrator_router.go)：OrchestratorWorkflow関数のルーティングswitch文を探して、「単純タスク」や「Swarmが必要」の判定方法、子ワークフローへの委譲を理解する
