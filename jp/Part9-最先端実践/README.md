...
> 最新のホットトピック：Deep Research、Computer Use、Agentic Coding、Background Agents、階層型モデル戦略

## 章一覧

| 章 | タイトル | コア課題 | Shannonとの関連 |
|------|------|----------|-------------|
| 27 | Deep Research | どうやって体系的な深層調査を実装するか？ | `roles/deep_research/`, `workflows/strategies/research.go` |
| 28 | Computer Use | どうやってエージェントがブラウザやデスクトップを操作するか？ | `config/models.yaml` multimodal |
...
> 「検索」から「調査」へ：AIをプロの研究者のように思考させる

**コアコンテンツ**:
- **本質的な違い**: より多く検索するのではなく、思考できること——計画、適応、検証、統合
- **2つのアーキテクチャ**: 単一エージェント（強力な推論）vs マルチエージェント（協調）
- **Anthropicアプローチ**: Orchestrator-Workerアーキテクチャ、コアインサイトは「圧縮」
- **コア決定事項**: アーキテクチャ選択、停止条件、引用検証、失敗回復
- **品質保証**: 引用追跡、カバレッジ評価、時間認識、多言語対応

**Shannonコード**: `roles/deep_research/deep_research_agent.py`, `workflows/strategies/research.go`
...
- [ ] Deep Researchのアーキテクチャ選択とコア設計決定を理解する
...
|------|----------|-------------|------|
| Deep Research | Perplexity、Gemini、ChatGPT | マルチエージェント + カバレッジ評価 | Ch27 |
| Computer Use | Claude Computer Use、Manus | マルチモーダル + ツール拡張 | Ch28 |
| Agentic Coding | Claude Code、Cursor、Windsurf | WASIサンドボックス + ファイルツール | Ch29 |
| Background Agents | Claude Code Ctrl+B | Temporal Schedule API | Ch30 |
| Cost Optimization | エンタープライズ向け低コスト化需要 | 階層型モデル戦略 | Ch31 |
...
| モジュール | 章 | 能力 |
|------|------|------|
| Deep Research | 第27章 | 体系的深層調査 |
| Computer Use | 第28章 | ウェブブラウジング、コンテンツ抽出 |
| Agentic Coding | 第29章 | 分析スクリプト生成 |
| Background Agents | 第30章 | 定期研究レポート |
| Tiered Models | 第31章 | インテリジェントモデル選択 |
